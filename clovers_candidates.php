...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    for ($i = 0; $i < count($candidateNames); $i++) {
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) AS count FROM clovers_candidates WHERE position = ? AND year_level = ?");
        ...
        $stmt = $conn->prepare("INSERT INTO clovers_candidates (name, position, year_level) VALUES (?, ?, ?)");
    ...
    header("clovers_view_scores.php");
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="clovers.css" />
</head>
<body>
  <div class="header">
    ...
    <div class="sidebar">
      ...
      <script>
        function Election_settings() {
          location.replace("clovers_settings.php");
        }
        function Candidates() {
          location.replace("clovers_candidates.php");
        }
        function ViewScores() {
          location.replace("clovers_view_scores.php");
        }
      </script>
    </div>
    <div class="candidates">
      <div class="resultExit">
        <a href="clovers_settings.php" id="return">Return to Elections</a>
      </div>
      <div class="candidateList">
        ...
        <div class="addCandidateForm">
          <form id="candidateForm" action="clovers_candidates.php" method="POST">
            <div class="field">
              <p>Name</p>
  ...
  <script>
    ...
    function saveCandidates() {
      ...
      xhr.open("POST", "clovers_candidates.php", true);
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState === XMLHttpRequest.DONE) {
          if (xhr.status === 200) {
            ...
            window.location.replace("clovers_view_scores.php");
          }
        }
      };
      xhr.send(
