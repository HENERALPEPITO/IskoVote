...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    for ($i = 0; $i < count($candidateNames); $i++) {
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) AS count FROM redbolts_candidates WHERE position = ? AND year_level = ?");
        ...
        $stmt = $conn->prepare("INSERT INTO redbolts_candidates (name, position, year_level) VALUES (?, ?, ?)");
    ...
    header("redbolts_view_scores.php");
...
<body>
    ...
    <div class="candidates">
      ...
      <div class="candidateList">
        ...
        <div class="addCandidateForm">
          <form id="candidateForm" action="redbolts_candidates.php" method="POST">
            <div class="field">
              <p>Name</p>
  ...
  <script>
    ...
    function saveCandidates() {
      ...
      xhr.open("POST", "redbolts_candidates.php", true);
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState === XMLHttpRequest.DONE) {
          if (xhr.status === 200) {
            ...
            window.location.replace("redbolts_view_scores.php");
          }
        }
      };
      xhr.send(
